...
{ 
	...
	{ 
	        ...
	        sleep(2); 
	        printf("philosophers %d is Pick The fork %d and %d\n", Phil_number + 1, Phil_left + 1, Phil_number + 1); 
...
{ 
    ...
    printf("philosophers %d Is Put The fork %d and %d down\n",Phil_number + 1, Phil_left + 1, Phil_number + 1); 
...
{ 
    ...
    for (i = 0; i < number; i++)
    { 
        pthread_create(&thread_id[i], NULL,Philos, &Phil_arr[i]); 
        printf("philosophers %d is thinking\n", i + 1); 
    } 
